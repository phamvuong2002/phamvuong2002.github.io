const url = "http://localhost:8080/api/driver/selectdoncho"
const url_updateTTCHO = "http://localhost:8080/api/driver/updatetinhtrangcho"
...
async function updateTinhtrangCHO(matx,madh){
    const newjsonObject = {
        "madh": madh,
        "matx": matx
    }
    const response = await fetch(url_updateTTCHO, {
    method: "POST",
...
function returnMenu(data){
    ...
    for(let i = 0; i < data.length; i++){
        ...
        button.onclick = function(){ updateTinhtrangCHO(jsondriver.MATX, data[i].MADONHANG)}
